...
export function calculateFullProfile({
  ...
  const lastOutTransaction =
    outTransactions.length > 0
      ? new Date(outTransactions[outTransactions.length - 1].timestamp * 1000)
      : undefined
  const firstInTransaction =
    inTransactions.length > 0 ? new Date(inTransactions[0].timestamp * 1000) : undefined
  const lastInTransaction =
    inTransactions.length > 0
      ? new Date(inTransactions[inTransactions.length - 1].timestamp * 1000)
      : undefined

  // Frequency
  const millisecondsPerDay = 1000 * 60 * 60 * 24
  const daysPassed =
    firstOutTransaction && lastOutTransaction
  ...
  const txs: SignerProfile['transactions'] = {
    ...
    in: {
      ...
      spanDays:
        firstInTransaction && lastInTransaction
